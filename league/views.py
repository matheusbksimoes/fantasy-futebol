...
def set_lineup(request, draft_id: int, team_id: int, week_number: int = None):
    ...
    if request.method == "POST":
        ...
        try:
            with transaction.atomic():
                original_lineup_player_ids = set(
                    LineupSpot.objects
                ...
                for (t, idx) in expected:
                    ...
                    if (
                        ...
                        and player_locked(player, round_number)
                        and player.id not in original_lineup_player_ids
                    ):
    ...
    try:
        ...
        for locked_player in locked_players:
            already_visible = any(
                slot.player and slot.player.id == locked_player.id
                for slot in slots
            )
            ...
            for slot in slots:
                if slot.slot_type != locked_player.position:
                    continue

                if slot.player and player_locked(slot.player, round_number):
                    continue

                slot.player = locked_player
                break
